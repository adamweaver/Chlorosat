...
3. ~~Adam deploys with `deploy.sh` if CS-024 auto-deploy isn't ready yet.~~ Auto-deploy is live (CS-024 done early, #7): merging updates chlorosat.com.
...
| CS-024 | Adam | ✅ **Done early (#7).** **CD (repo side):** `.github/workflows/deploy.yml` per [DEPLOYMENT.md](../DEPLOYMENT.md#automatic-deploy-sprint-3-githubworkflowsdeployyml) (`production` environment, rsync with the locked key, [security rules](../DEPLOYMENT.md#security)); CI caching + path filters (**per job**, never a workflow-level `paths:`, because `pipeline` + `web` are required checks) | 8 | Merging a PR updates the live site automatically in < 5 min | CS-013; CS-027 (Adam adds the environment + secrets) |
...
| CS-027 | Adam | **Server (Adam only):** GitHub `production` environment + secrets; repo security settings (fork-PR approval, secret scanning + push protection: ✅ done 2026-09-24); nginx cache headers (+ check gzip); watch the first automatic deploy | 2 | Environment + secrets exist; cache headers show in `curl -I https://chlorosat.com/data/manifest.json` | CS-013 |
