...
| CS-027 | Adam | **Server (Adam only):** GitHub `production` environment + secrets; nginx cache headers (+ check gzip); watch the first automatic deploy | 2 | Environment + secrets exist; cache headers show in `curl -I https://<domain>/data/manifest.json` | CS-013 |
